...
class Installer:
    ...
    def install(self):
        ...
        self.install_links(
            self.config_root.joinpath("dotfiles"),
            self.homedir,
            transform=self.tr_dotfile,
        )
...
def main():
    ...
    parser.add_argument(
        "-r", "--relative", action="store_true", help="make relative symlinks"
    )
    parser.add_argument(
        "-p", "--preserve", action="store_true", help="rename existing files to `*.bak`"
    )
    parser.add_argument(
        "-d", "--dryrun", "--dry-run", action="store_true", help="look but don't touch"
    )
