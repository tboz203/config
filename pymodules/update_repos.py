...
def update_branch(repo: git.Repo, branch: git.Head, lprune: bool = False) -> None:
    ...
    is_current_branch: bool = (not repo.head.is_detached) and repo.head.ref == branch
    tracking_branch = branch.tracking_branch()
    if tracking_branch is None:
        logger.info("no tracking branch set: %s %s", repo.working_dir, branch)
    elif not tracking_branch.is_valid():
        if (not is_current_branch) and lprune:
            logger.info("Pruning local branch: %s %s", repo.working_dir, branch)
            repo.delete_head(branch, force=True)
        else:
            logger.warning(
                "Failed to update branch; tracking branch gone upstream: %s %s",
                repo.working_dir,
                branch,
            )
    elif not repo.is_ancestor(branch, tracking_branch):
        logger.warning("Failed to update branch; not fast-forward: %s %s", repo.working_dir, branch)
    elif repo.is_ancestor(tracking_branch, branch):
        logger.debug("no changes for branch: %s %s", repo.working_dir, branch)
    elif is_current_branch:
        if repo.is_dirty():
            logger.warning('Refusing to update checked out branch: local changes would be overwritten')
        else:
            repo.head.reset(tracking_branch, index=True, working_tree=True)
    else:
        branch.commit = tracking_branch.commit
        logger.info("Updated branch: %s %s", repo.working_dir, branch)
