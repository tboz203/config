...
function from_list {
    ...
    # shellcheck disable=SC2178  # "used as array but assigned string" it's a nameref
    local -n arrayref=$1
    local list=$2
...
function pause {
    if [[ $# -gt 1 || ${1-} == -* ]]; then
        echo "$FUNCNAME: pause for user input"
        echo "Usage: $FUNCNAME [PROMPT]"
    fi
    local rc=0 REPLY
    read -rsp "${1-[Press enter to contine]} " || rc=$?
    echo
    return $rc
}
