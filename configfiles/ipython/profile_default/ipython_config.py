...
def _text_repr_pretty_(value: AnyStr, pp: pretty.RepresentationPrinter, cycle: bool) -> None:
    ...
    if cycle:
        pp.text(f"{type(value).__name__}(...)")
        return

    def rfind_before_escape_sequence(line: str, boundary: int) -> Optional[int]:
        """
        Find the index in `line` before an escape sequence straddling `boundary`, if any.
        """
        # order is important! if \\ is not last, it beats other conflicting
        # escape sequences!
        escapes = [
            ("\\U", 8),
            ("\\u", 4),
        ...
        for leader, dist in escapes:
            # find bounds in which this sequence would cause problems
            start, stop = boundary - dist, boundary
            if leader in line[start:stop]:
                # there's a sequence of this form straddling the boundary; find and return the index
                return line.rfind(leader, start, stop)

        # no escapes at the boundary
        return None

    def rfind_whitespace(line: str, start: int, stop: int) -> Optional[int]:
    ...
    def get_index_for_width(line: str, width: int) -> Optional[int]:
        """
        Find the index in "repr" `line` that results in the largest prefix of
        `line` that is at most `width` character-widths wide. If the entire
        `line` fits, return `None`.
        """
        if width <= 0:
            return 0
        ...
        for idx, ch in enumerate(line):
            if east_asian_width(ch) in {"F", "W", "A"}:
                # full, wide, and ambiguous characters count for two
                adjusted += 2
            else:
                adjusted += 1
            if adjusted >= width:
        ...
        # never got to requested width!
        return None

    def chunks(text: str, width: int) -> list[str]:
        """Intelligently break a "repr" `line` into chunks of max width `width`."""
        broken: list[str] = []
        while True:
            # find the max index in `text` that fits on one line, or `None` if
            # the whole text fits
            boundary = get_index_for_width(text, width)

            if (idx := text.find("\\n", 0, boundary or width)) >= 0:
                # always break on newlines, whether it fits on one line or not
                boundary = idx + 2
                ...
                # either it all fits (boundary is None; `text` will be "" after
                # split), or we're fine to break at `width`
                boundary = width

            chunk, text = text[:boundary], text[boundary:]
            broken.append(chunk)
            if not text:
                break
    ...
    # strip off & store delimiters
    quote = repr_value[-1]
    if repr_value[0] == quote:
        prefix = ""
        repr_value = repr_value[1:-1]
    else:
        prefix = repr_value[0]
        repr_value = repr_value[2:-1]

    # maximum permissible width of our repr value
    max_width = pp.max_width - pp.indentation - len(prefix + quote + quote)

    if get_index_for_width(repr_value, max_width) is None:
        # the whole value fits on one line
        pp.text(f"{prefix}{quote}{repr_value}{quote}")
        return

    # our wrapping '()'
    max_width -= 2

    lines = chunks(repr_value, max_width)

    with pp.group(open="(", close=")"), pp.indent(1):
...
def _sequence_repr_pretty(value: Sequence, pp: pretty.RepresentationPrinter, cycle: bool) -> None:
    ...
    with pp.group(open=open, close=close), pp.indent(1):
...
#     {
#         str: _text_repr_pretty_,
#         bytes: _text_repr_pretty_,
#         list: _sequence_repr_pretty,
#         tuple: _sequence_repr_pretty,
#     }
# )
...
#  Choices: any of [
#    'auto', 'agg', 'gtk', 'gtk3', 'gtk4', 'inline', 'ipympl', 'nbagg', 'notebook', 'osx', 'pdf', 'ps', 'qt', 'qt4',
#    'qt5', 'qt6', 'svg', 'tk', 'webagg', 'widget', 'wx'
#  ] (case-insensitive) or None
#  Default: None
...
#  Default: """Python 3.11.6 (main, Nov  9 2023, 17:45:30) [GCC 11.2.0]
#  Type 'copyright', 'credits' or 'license' for more information
#  IPython 8.17.2 -- An enhanced Interactive Python. Type '?' for help.
#  """
# c.InteractiveShell.banner1 = """Python 3.11.6 (main, Nov  9 2023, 17:45:30) [GCC 11.2.0]
# Type 'copyright', 'credits' or 'license' for more information
# IPython 8.17.2 -- An enhanced Interactive Python. Type '?' for help.
# """
...
# c.TerminalInteractiveShell.banner1 = """Python 3.11.6 (main, Nov  9 2023, 17:45:30) [GCC 11.2.0]
# Type 'copyright', 'credits' or 'license' for more information
# IPython 8.17.2 -- An enhanced Interactive Python. Type '?' for help.
# """
